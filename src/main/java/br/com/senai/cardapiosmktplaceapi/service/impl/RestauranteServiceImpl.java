...
public class RestauranteServiceImpl implements RestauranteService {
	...
	@Autowired
	private CardapiosRepository cardapiosRepositoy;
	...
	public Restaurante salvar(Restaurante restaurante) {
		...
		if (outroRestaurante != null) {
			if (restaurante.isPersistido()) {
				Preconditions.checkArgument(outroRestaurante.equals(restaurante), "O nome do restaurante ja esta em uso");
			}
		}
	...
	public void atualizarStatusPor(Integer id, Status status) {
		...
		Preconditions.checkArgument(restauranteEncontrado.getStatus() != status, "O status ja está salvo para o restaurante");
	...
	public Restaurante buscarPor(Integer id) {
		Restaurante restauranteEncontrado = repository.buscarPor(id);
		Preconditions.checkNotNull(restauranteEncontrado, "Não foi encontrado o restaurante para o id informado");
		Preconditions.checkArgument(restauranteEncontrado.isAtivo(), "O restaurante está inativo");
	...
	public Restaurante excluirPor(Integer id) {
		...
		Long qtdeDeCardapiosVinculados = cardapiosRepositoy.contarPor(id);
		Preconditions.checkArgument(qtdeDeCardapiosVinculados == 0, "Existem cardápios vinculados a esse restaurante");
