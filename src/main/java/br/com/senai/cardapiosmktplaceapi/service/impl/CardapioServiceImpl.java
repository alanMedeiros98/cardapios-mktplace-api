...
public class CardapioServiceImpl implements CardapioService {
	...
	private Restaurante getRestaurantePor(NovoCardapio novoCardapio) {
		...
		Preconditions.checkNotNull(restaurante, "O restaurante '" + novoCardapio.getRestaurante().getId() + "' não foi salvo");
	...
	private Secao getSecaoPor(NovaOpcaoDoCardapio novaOpcao) {
		...
		Preconditions.checkNotNull(secao, "Não existe seção vinculada ao id'" + novaOpcao.getSecao().getId() + "'");
	...
	private Opcao getOpcaoPor(Integer idDaOpcao, Restaurante restaurante) {
		...
		Preconditions.checkArgument(opcao.getRestaurante().equals(restaurante), "A opção '" + idDaOpcao + "' não pertence ao restaurante do cardápio");
		return opcao;
	}
	...
	private void validarDuplicidadeEm(List<NovaOpcaoDoCardapio> opcoesDoCardapio) {
		for (NovaOpcaoDoCardapio novaOpcao : opcoesDoCardapio) {
			...
			Preconditions.checkArgument(qtdeDeOcorrencias == 1, "A opção '" + novaOpcao.getIdDaOpcao() + "' está duplicada no cardápio");
	...
	public Cardapio alterar(CardapioSalvo cardapioSalvo) {
		...
		Preconditions.checkArgument(restaurante.equals(cardapio.getRestaurante()), "O restaurante do cardápio não pode ser alterado");
	...
	private void atualizarPrecosDas(List<OpcaoDoCardapio> opcoesDoCardapio) {
		for(OpcaoDoCardapio opcaoDoCardapio : opcoesDoCardapio) {
			if (opcaoDoCardapio.getOpcao().isEmpromocao()) {
				...
				BigDecimal valorDescontado = opcaoDoCardapio.getPreco().multiply(percentualDeDesconto).divide(divisor);
	...
	public Cardapio buscarPor(Integer id) {
		...
		Preconditions.checkNotNull(cardapioEncontrado, "Não foi encontrado cardapio para id informado");
		Preconditions.checkArgument(cardapioEncontrado.isAtivo(), "O cardapio esta inativo");
	...
	public void atualizarStatusPor(Integer id, Status status) {
		...
		Preconditions.checkNotNull(cardapio, "Não foi encontrado cardapio para id informado");
