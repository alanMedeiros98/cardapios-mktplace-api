...
public interface CategoriasRepository extends JpaRepository<Categoria, Integer>{
	...
	@Query(value = "SELECT c FROM Categoria c WHERE Upper(c.nome) LIKE Upper(:nome) AND c.status = :status AND c.tipo = :tipo ORDER BY c.nome ", countQuery = "SELECT Count(c) FROM Categoria c WHERE Upper(c.nome) LIKE Upper(:nome) AND c.status = :status AND c.tipo = :tipo ")
